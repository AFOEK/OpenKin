...
class Persons(db.Model):
    ...
    create_by_user_id = db.Column(db.String(36), db.ForeignKey('users.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Relationship(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    from_person_id = db.Column(db.String(36), db.ForeignKey('persons.id'), nullable=False)
    to_person_id = db.Column(db.String(36), db.ForeignKey('persons.id'), nullable=False)
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_by_user_id = db.Column(db.String(36), db.ForeignKey('users.id'))
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
